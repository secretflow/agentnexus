...
import { type ChatConfigProps, ChatConfigSchema } from "@/lib/zod";
...
export function RecallConfigForm({
  ...
  onSuccess?: (data: ChatConfigProps["recall"]) => void;
  onClose?: () => void;
}) {
  ...
  } = useForm<ChatConfigProps["recall"]>({
  ...
  const onSubmit = handleSubmit(async (data: ChatConfigProps["recall"]) => {
    onSuccess?.({
      ...data,
      semantics,
    });
